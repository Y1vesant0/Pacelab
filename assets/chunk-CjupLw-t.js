...
// ── Column name mappings (normalized) ────────────────────────
// Normalization: lowercase, trim, remove spaces, remove (), remove unit suffixes
const COL_DATE = ['date', '날짜', 'rundate', 'workoutdate'];
const COL_DISTANCE = ['distance', '거리km', '거리', '총거리', '운동거리'];
const COL_DURATION = ['duration', 'time', '운동시간', '경과시간', '총시간', 'movingtime', 'elapsedtime', 'totaltime'];
const COL_PACE = ['pace', 'avgpace', 'averagepace', '평균페이스', '페이스'];
const COL_HR = ['avghr', 'averagehr', 'averageheartrate', 'heartrate', '평균심박', '심박', '심박수', '평균심박수'];
const COL_MAX_HR = ['maxhr', 'maximumhr', '최고심박', '최고심박수'];
const COL_CADENCE = ['cadence', 'avgcadence', '평균케이던스', '케이던스', '케이던스spm'];
const COL_POWER = ['power', 'avgpower', '평균파워', '파워', '평균파워w'];
const COL_ELEVATION = ['elevation', 'gain', '등반고도', '등반고도m', '상승고도', '고도'];
const COL_STRIDE = ['stride', '보폭', '보폭m'];
const COL_GCT = ['gct', 'groundcontact', '지면접촉', '지면접촉ms'];
const COL_VO = ['vo', 'verticaloscillation', '수직진폭', '수직진폭cm'];
const COL_SHOE = ['shoe', '신발'];
const COL_NOTES = ['notes', 'memo', '메모', '비고', '노트', 'comment'];
...
function findColumnIndex(headers, candidates) {
    for (let i = 0; i < headers.length; i++) {
        if (matchColumn(headers[i], candidates))
            return i;
    }
    return -1;
}
...
function parseDistance(val) {
    if (val == null)
        return undefined;
    const s = String(val).replace(/\s*km\s*$/i, '').replace(/,/g, '.').trim();
...
function parseDurationValue(val) {
    ...
    // hh:mm:ss or mm:ss (e.g., "0:41:42", "1:09:57", "52:30")
    const colons = s.match(/^(\d{1,2}):(\d{2})(?::(\d{2}))?$/);
    if (colons) {
        const parts = colons.slice(1).filter(Boolean).map(Number);
        if (parts.length === 3)
            return parts[0] * 3600 + parts[1] * 60 + parts[2];
        if (parts.length === 2)
            return parts[0] * 60 + parts[1];
    }
    // "52 min 30 sec" or "52분 30초"
    const minSec = s.match(/(\d+)\s*(?:min|분)\s*(?:(\d+)\s*(?:sec|초))?/i);
    if (minSec) {
        const m = parseInt(minSec[1], 10);
        const sec = minSec[2] ? parseInt(minSec[2], 10) : 0;
        return m * 60 + sec;
    }
    // Plain number — interpret as seconds if > 200, minutes otherwise
    const n = parseFloat(s.replace(/,/g, '.'));
    if (!isNaN(n) && n > 0) {
        return n > 200 ? Math.round(n) : Math.round(n * 60);
    }
    return undefined;
}
function parsePaceValue(val) {
    if (val == null)
        return undefined;
    const s = String(val).trim();
    // 5:12 or 5'12" or 5:12/km or 5분 12초
    const m1 = s.match(/(\d{1,2})[:'](\d{2})(?:["″])?(?:\s*\/\s*km)?/);
    if (m1) {
        const sec = parseInt(m1[1], 10) * 60 + parseInt(m1[2], 10);
        if (sec >= 120 && sec <= 1500)
            return sec;
    }
    const m2 = s.match(/(\d{1,2})분\s*(\d{1,2})초/);
    if (m2) {
        const sec = parseInt(m2[1], 10) * 60 + parseInt(m2[2], 10);
        if (sec >= 120 && sec <= 1500)
            return sec;
    }
    // Numeric — e.g. 5.2 means 5:12
    const n = parseFloat(s.replace(/,/g, '.'));
    if (!isNaN(n) && n >= 2 && n <= 25) {
...
function parseHR(val) {
    if (val == null)
        return undefined;
    const s = String(val).replace(/\s*bpm\s*$/i, '').trim();
    const n = parseInt(s, 10);
    if (isNaN(n) || n < 40 || n > 230)
        return undefined;
    return n;
}
function parseCadence(val) {
    if (val == null)
        return undefined;
    const n = parseInt(String(val).trim(), 10);
    if (isNaN(n) || n < 80 || n > 260)
        return undefined;
    return n;
}
function parseDateValue(val) {
    ...
    const s = String(val).trim();
    // Excel serial date number (e.g., 45432)
    if (/^\d{5}$/.test(s)) {
    ...
    // Dots: 2026.05.18
    const dots = s.match(/^(\d{4})[./](\d{1,2})[./](\d{1,2})/);
    if (dots)
        return `${dots[1]}-${dots[2].padStart(2, '0')}-${dots[3].padStart(2, '0')}`;
    // JS Date fallback
    const d = new Date(s);
    if (!isNaN(d.getTime()))
        return fmt(d);
...
async function parseExcelFile(file) {
    try {
        ...
        const wb = readSync(buf, { type: 'array', cellDates: true });
        if (wb.SheetNames.length === 0) {
            return { rows: [], totalRows: 0, validRows: 0, invalidRows: 0, error: 'No sheets found.' };
        }
        // Prioritize "운동기록" sheet
        let sheetName = wb.SheetNames[0];
        const preferred = wb.SheetNames.find(n => n === '운동기록' || n.toLowerCase() === 'workout');
        if (preferred)
            sheetName = preferred;
        const sheet = wb.Sheets[sheetName];
        const json = utils.sheet_to_json(sheet, { defval: '' });
        if (json.length === 0) {
            return { rows: [], totalRows: 0, validRows: 0, invalidRows: 0, error: 'Sheet is empty.' };
        }
        // Detect columns from first row keys
        const headers = Object.keys(json[0]);
        const iDate = findColumnIndex(headers, COL_DATE);
        const iDist = findColumnIndex(headers, COL_DISTANCE);
        const iDur = findColumnIndex(headers, COL_DURATION);
        const iPace = findColumnIndex(headers, COL_PACE);
        const iHR = findColumnIndex(headers, COL_HR);
        const iMaxHR = findColumnIndex(headers, COL_MAX_HR);
        const iCadence = findColumnIndex(headers, COL_CADENCE);
        const iPower = findColumnIndex(headers, COL_POWER);
        const iElevation = findColumnIndex(headers, COL_ELEVATION);
        const iStride = findColumnIndex(headers, COL_STRIDE);
        const iGct = findColumnIndex(headers, COL_GCT);
        const iVo = findColumnIndex(headers, COL_VO);
        const iShoe = findColumnIndex(headers, COL_SHOE);
        const iNotes = findColumnIndex(headers, COL_NOTES);
        const rows = [];
        for (let i = 0; i < json.length; i++) {
            const raw = json[i];
            const vals = headers.map(h => raw[h]);
            const date = iDate >= 0 ? parseDateValue(vals[iDate]) : undefined;
            const distance = iDist >= 0 ? parseDistance(vals[iDist]) : undefined;
            let duration = iDur >= 0 ? parseDurationValue(vals[iDur]) : undefined;
            let pace = iPace >= 0 ? parsePaceValue(vals[iPace]) : undefined;
            const avgHR = iHR >= 0 ? parseHR(vals[iHR]) : undefined;
            const maxHR = iMaxHR >= 0 ? parseHR(vals[iMaxHR]) : undefined;
            const cadence = iCadence >= 0 ? parseCadence(vals[iCadence]) : undefined;
            const power = iPower >= 0 ? parseFloat(String(vals[iPower]).replace(/,/g, '.')) : undefined;
            const elevation = iElevation >= 0 ? parseFloat(String(vals[iElevation]).replace(/,/g, '.')) : undefined;
            const stride = iStride >= 0 ? parseFloat(String(vals[iStride]).replace(/,/g, '.')) : undefined;
            const groundContact = iGct >= 0 ? parseInt(String(vals[iGct]).trim(), 10) : undefined;
            const verticalOscillation = iVo >= 0 ? parseFloat(String(vals[iVo]).replace(/,/g, '.')) : undefined;
            const shoe = iShoe >= 0 ? String(vals[iShoe] ?? '').trim() : undefined;
            const notes = iNotes >= 0 ? String(vals[iNotes] ?? '').trim() : undefined;
            // Calculate missing pace or duration
            if (!pace && distance && duration && distance > 0) {
                pace = Math.round(duration / distance);
            }
            if (!duration && distance && pace && distance > 0) {
                duration = Math.round(pace * distance);
            }
            // Determine status
            const hasDate = !!date;
            const hasDist = distance !== undefined && distance > 0;
            const hasDurOrPace = (duration !== undefined && duration > 0) || (pace !== undefined && pace > 0);
            let status = 'ready';
            let statusDetail;
            if (!hasDate && !hasDist && !hasDurOrPace) {
                status = 'invalid';
                statusDetail = 'No data';
            }
            else if (!hasDate || !hasDist) {
                status = 'missing';
                statusDetail = !hasDate ? 'Missing date' : 'Missing distance';
            }
            else if (!hasDurOrPace) {
                status = 'missing';
                statusDetail = 'Missing duration';
            }
            rows.push({
                rowIndex: i + 2,
                date,
        ...
        const validRows = rows.filter(r => r.status === 'ready').length;
        const invalidRows = rows.length - validRows;
        return { rows, totalRows: rows.length, validRows, invalidRows };
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : 'Unknown error';
        return { rows: [], totalRows: 0, validRows: 0, invalidRows: 0, error: msg };
    }
}
